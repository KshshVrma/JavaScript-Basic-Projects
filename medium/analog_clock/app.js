...
function AnalogClock(id, option) {
  ...
  if (!option) option = {}; //avoid undefined exception
  ...
  for (var i = 0; i <= 5; i++) {
    ...
    if (this.width < 100) numTop.innerHTML = "●";
    ...
    if (this.width < 100) numBottom.innerHTML = "●";
  ...
  if (localStorage) this.panel.appendChild(min);
  ...
  sec.style.borderTop =
    "solid " + (this.width * 0.5 - secTop) + "px " + this.foreColor;
  sec.style.borderBottomWidth = this.width * 0.5 - secTop + "px";
  if (localStorage) this.panel.appendChild(sec);
  ...
  if (localStorage) this.panel.appendChild(point);
  ...
  this.loop = setInterval(function () {
    ...
    var roH =
      ((1.0 * 360) / 12) * (now.getHours() % 12) +
      ((1.0 * 360) / 12) * (now.getMinutes() / 60);
